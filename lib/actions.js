...
exports.wait = function (arg, ...args) {
  if (args.length === 0) {
  ...
  if (typeof arg === 'number') {
    ...
    if (args.length - arg.length >= 1 && typeof args[0] === 'number') {
      this.timeout = timeout = args.splice(0, 1)[0];
    }
    debug('.wait() for fn'+(timeout ? ' or '+timeout+'msec' : ''));
    waitfn.apply(this, [this, arg, done, args]);
  }
  else {
    done();
  }
...
function waitfn(self, fn, done, args) { 
  var timeout = this.timeout || null;
  var waitMsPassed = 0;
  return tick.call(self, args);
  ...
  function tick (args) {
    ...
    self.evaluate_now.apply(self, [fn, waitDone].concat(args));
  }
}
